# Fork of libgit2 to perform advanced synchronization with remotes using custom backends

## The Problem

How to get the latest data from remote git repositories on a large
scale?
...
This is focused on getting objects that are not in WoC yet
and assumes that we have identified new/updated repositories 
using code with swsc/gather (discovery) stage.

The script provided here:
* captureObjects.sh clones the repositories and lists objects in them. It also checks which objects are missing, and then extracts them
...
and three orders of magnitude more and more network bandwith and
storage.


What needs to be done is, as in case of git_last, insert
additional logic to git fetch protocol in order to use
custom backend that comprises git objects from all repositiories
and not from a single repository as git fetch assumes.
git_last implements the first step in git fetch protocol which
